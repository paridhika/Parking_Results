...
public class Correlogram_MQTT_80 {

	private static BufferedReader delete,put,get;

	public static void main(String[] args) throws IOException {
		delete = new BufferedReader(new FileReader("/home/paridhika/git/MQTT_Parking/mosquitto/Results/run1/80/delete_simulation_service_time_80_run1.csv"));
		put = new BufferedReader(new FileReader("/home/paridhika/git/MQTT_Parking/mosquitto/Results/run1/80/put_simulation_service_time_80_run1.csv"));
		get = new BufferedReader(new FileReader("/home/paridhika/git/MQTT_Parking/mosquitto/Results/run1/80/get_simulation_service_time_80_run1.csv"));
		...
		/*FileWriter correlation = new FileWriter("/home/paridhika/git/MQTT_Parking/mosquitto/Results/Correlogram_80.csv");
...
/*		double numerator = 0;
for(int i = 0;i < r1.size();i++){
	numerator += 1-((i+1)*r1.get(i)/w1.size());
}
SD[0] = SD[0] * Math.sqrt(1 + numerator*2);
System.out.println(Math.sqrt(1 + numerator*2));
numerator = 0;
for(int i = 0;i < r2.size();i++){
	numerator += 1-((i+1)*r2.get(i)/w2.size());
}
SD[1] = SD[1] * Math.sqrt(1 + numerator*2);
numerator = 0;
for(int i = 0;i < r3.size();i++){
	numerator += 1-((i+1)*r3.get(i)/w3.size());
}
SD[2] = SD[2] * Math.sqrt(1 + numerator*2);*/